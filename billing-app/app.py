...
def challan():
    ...
    company = firms.get(chosen_firm_key, next(iter(firms.values()))) if firms else {
        "title_name":"", "company_name":"", "addr":"", "mobile":"", "gst":"", "bank_lines":[]
    }
    ...
    ch_dt  = request.form.get("challan_date") or datetime.now().strftime("%d/%m/%Y")
    ...
    # Create PDF to memory
    buf = io.BytesIO()
    draw_challan_pdf(
        buf,
        company=company,
        party=party,
        meta={"no": ch_no, "date": ch_dt},
    ...
    for d, q, r, a in items:
        row = [
            ...
            created,
            ch_dt,          # Invoice_Date column name kept as per your sheet
            ch_no,
            party_code,
            party["name"],
            party["gstin"],
        ...
        append_row_to_challan(row)

    filename = _unique_name(f"Challan_{party['name'] or 'Party'}_{ch_no}.pdf")
...
def invoice():
    ...
    inv_dt = request.form.get("invoice_date") or datetime.now().strftime("%d/%m/%Y")
